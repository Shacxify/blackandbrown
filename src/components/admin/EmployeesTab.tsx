...
const EmployeesTab = ({ users, listLoading, reload }: Props) => {
  const { user } = useAuth();
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setUsername(''); setFullName(''); setPassword(''); setRole('employee');
    reload();
  };
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleCreate} className="space-y-4">
            <div>
              <Label htmlFor="new-username" className="text-xs tracking-wide uppercase">Username</Label>
              <Input id="new-username" value={username} onChange={(e) => setUsername(e.target.value)}
                placeholder="e.g. jane" className="mt-1.5" autoComplete="off" required />
            </div>
            <div>
              <Label htmlFor="new-name" className="text-xs tracking-wide uppercase">Full Name</Label>
              <Input id="new-name" value={fullName} onChange={(e) => setFullName(e.target.value)}
                placeholder="Jane Doe" className="mt-1.5" autoComplete="off" />
            </div>
            <div>
              <Label htmlFor="new-password" className="text-xs tracking-wide uppercase">Password</Label>
              <Input id="new-password" type="password" value={password}
                onChange={(e) => setPassword(e.target.value)} placeholder="Minimum 8 characters"
                className="mt-1.5" autoComplete="new-password" required minLength={8} />
            </div>
            <div>
                      ...
                      <TableCell>
                        {u.roles.map((r) => (
                          <Badge key={r} variant={r === 'admin' ? 'default' : 'secondary'}
                            className="mr-1 text-[10px] uppercase">{r}</Badge>
                        ))}
                      </TableCell>
